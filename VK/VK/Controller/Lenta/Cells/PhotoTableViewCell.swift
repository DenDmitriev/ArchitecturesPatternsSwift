...
class PhotoTableViewCell: UITableViewCell, PostSet {
    ...
    fileprivate func configurate() {
        ...
        collectionView.layoutMargins = UIEdgeInsets(top: Size.Post.zero, left: Size.Post.zero, bottom: Size.Post.zero, right: Size.Post.zero)
        collectionView.contentInset = UIEdgeInsets(top: Size.Post.zero, left: Size.Post.zero, bottom: Size.Post.zero, right: Size.Post.zero)
...
extension PhotoTableViewCell: UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, insetForSectionAt section: Int) -> UIEdgeInsets {
        return UIEdgeInsets(top: Size.Post.zero, left: Size.Post.zero, bottom: Size.Post.zero, right: Size.Post.zero)
    }
    
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, minimumInteritemSpacingForSectionAt section: Int) -> CGFloat {
        return Size.Post.seporator
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        case .double:
            let scale = self.bounds.width / ((photos.first!.width ?? self.bounds.width) + Size.Post.seporator + (photos.last!.width ?? self.bounds.width))
            height = (photos.first!.height ?? self.bounds.height) * scale
            width = scale * (photos[indexPath.item].width ?? self.bounds.width)
        default:
            height = Size.Post.photo
