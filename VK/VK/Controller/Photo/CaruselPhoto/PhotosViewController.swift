...
class PhotosViewController: UIViewController {
    ...
    func gestureAniamtionSetup(direction: Direction, nextIndex: Int) {
        ...
        switch direction {
        case .next:
            assistImageView.center.x += (view.frame.width + Size.Carusel.separator)
            move = -(view.frame.width + Size.Carusel.separator)
        case .previous:
            assistImageView.center.x -= (view.frame.width + Size.Carusel.separator)
            move = +(view.frame.width + Size.Carusel.separator)
        }
        ...
        animator = UIViewPropertyAnimator(duration: Size.Carusel.duration, curve: .easeInOut) {
    ...
    func toDefault(direction: Direction) {
        UIView.animate(withDuration: 0.25, animations: {
            self.imageView.center = self.defaultCenter
            switch direction {
            case .next: self.assistImageView.center.x = self.view.frame.width * 3/2 + Size.Carusel.separator
            case .previous: self.assistImageView.center.x = -self.view.frame.width/2 - Size.Carusel.separator
            }
        }) { _ in
            self.assistImageView.removeFromSuperview()
            self.assistImageView.center = self.defaultCenter
