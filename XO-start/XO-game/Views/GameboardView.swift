...
public class GameboardView: UIView {
    ...
    public func freePlaceMarkView() -> [GameboardPosition] {
        var freePostions: [GameboardPosition] = [
        ...
        let holdPositions = markViewForPosition.compactMap { $0.key }
        freePostions.removeAll { (position) -> Bool in
            var output = false
            holdPositions.forEach { (holdPosition) in
                if holdPosition == position {
                    output = true
                    return
                }
            }
